...
"SELECT facs.name, SUM(CASE WHEN memid =0 THEN slots * facs.guestcost ELSE slots * membercost END )
AS revenue FROM Bookings bks INNER JOIN Facilities facs ON bks.facid = facs.facid GROUP BY facs.name 
